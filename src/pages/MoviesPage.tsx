...
export function MoviesPage() {
  const { trending, popularMovies, fetchHomeData } = useMediaStore()
  ...
  const moviesTrending = trending.filter(m => m.media_type === 'movie')
  ...
  return (
    <>
        ...
        <div className="relative z-[2] h-full flex items-end pb-[80px]">
          <div className="w-[1400px] max-w-[95%] mx-auto">
            <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="max-w-[650px]">
              <span className="inline-block bg-[#1a1a1a] px-[18px] py-[10px] rounded-full text-xs tracking-[1px] uppercase">Filmes</span>
              ...
              <Link to={`/movie/${hero.id}`} className="inline-flex h-[56px] px-[35px] rounded-full bg-white text-black font-semibold no-underline items-center justify-center gap-2 text-sm mt-[25px] hover:bg-white/90 transition-colors"><PlayIcon size={16} color="#000" /> Assistir Agora</Link>
            </motion.div>
          </div>
        </div>
      </section>
      ...
      <div className="w-[1400px] max-w-[95%] mx-auto relative z-10 -mt-[60px]">
        {moviesTrending.length > 0 && <MediaRow title="Em Alta" items={moviesTrending} />}
        {popularMovies.length > 0 && <MediaRow title="Populares" items={popularMovies} />}
              ...
              return (
                <Link key={m.id} to={`/movie/${m.id}`} className="bg-[#111] rounded-[24px] overflow-hidden no-underline group">
                  ...
                  <div className="p-[25px]">
                    <h3 className="mb-[10px] text-lg text-white">{m.title}</h3>
